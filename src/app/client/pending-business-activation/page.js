...
export default function ClientPendingBusinessActivationPage() {
    ...
    const loadPageData = async () => {
      ...
      if (currentUser) {
        ...
        if (currentBusiness && currentBusiness.businessId) {
          ...
          // If business is now active, redirect to dashboard
  ...
  const handleCheckStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (updatedBusiness && updatedBusiness.businessId?.status === 'active') {
          // Update local storage with new user data
          localStorage.setItem('userData', JSON.stringify(updatedUser));
          router.push('/client/dashboard');
        } else {
          setError(`The business "${updatedBusiness?.businessId?.name || 'your business'}" is still ${updatedBusiness?.businessId?.status || 'pending'}. Please try again later.`);
          setBusinessInfo(prev => ({ ...prev, status: updatedBusiness?.businessId?.status || 'pending' }));
        }
      } else {
  ...
  const getStatusMessage = (status) => {
    switch (status) {
      case 'pending':
        return 'pending activation by our super admin team';
      case 'suspended':
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-8 md:p-12 w-full max-w-md text-center">
          ...
          <div className="space-y-4">
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {checkStatusLoading ? 'Checking Status...' : 'Check Business Status'}
            </button>
