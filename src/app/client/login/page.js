...
export default function ClientLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Store token and user data, then redirect to client dashboard
        ...
        localStorage.setItem('userData', JSON.stringify({
          ...
          clientStatus: result.clientStatus || 'approved' // Default to approved for successful logins
        }));
        window.location.href = '/client/dashboard';
      } else {
        // Handle business status issues
        if (result.businessStatus) {
          // Store token and user data even for pending/suspended businesses
          localStorage.setItem('authToken', result.token);
          localStorage.setItem('userData', JSON.stringify({
            _id: result._id,
            email: result.email,
          ...
          // Redirect to pending business activation page
          window.location.href = '/client/pending-business-activation';
        } else if (result.clientStatus) {
          // Handle client status issues (pending/suspended)
          if (result.clientStatus === 'pending') {
            // Store token and user data for pending clients
            localStorage.setItem('authToken', result.token);
            localStorage.setItem('userData', JSON.stringify({
              _id: result._id,
              email: result.email,
